...
storm <- read_csv(here::here("week2", "data", "la_storm.csv"))

# Create a plot of the gage height and precipitation data

ggplot(storm, aes(precip_wk_in, gage_ht)) +
  geom_point() +
  labs(x = "Weekly Precipitation (in)",
       y = "Gage Height")
```
...
storm_mod <- lm(gage_ht~precip_wk_in, data = storm)
...
storm_std <-storm_sum$sigma

storm_b0 <- storm_sum$coefficients[1,1]
storm_b1 <- storm_sum$coefficients[2,1]
```
...
beta 0 hat: when there is no rain, the lowest the gage height sits is 3.9

beta 1 hat: for every additional inch of rain the height, the gage increases by beta 1 hat
...
y_bar <- mean(storm$gage_ht)
x <- storm$precip_wk_in
y_hat <- 3.9 + 2.2 * x
SST <- sum((storm$gage_ht - y_bar)^2)
SSE <- sum((storm$gage_ht - y_hat)^2)
R2 <- (SST - SSE) / SST

# What fraction of the gage height have we explained using precipitation?
R2
...
y is the observed gage height, y_bar is the mean gage height, and y hat is the expected gage height

**Q5: What does the** $R^2$ **value tell you about the quality of the model (in plain language)?**

Precipitation only explains about half of the gage height
