...
penguins_sum <- penguins %>% 
  count(species, island) %>% 
  group_by(species) %>% 
  mutate(prop_pen = n / sum(n)) %>% 
  ungroup() %>% 
  group_by(island) %>% 
  mutate(prop_isl = n / sum(n))
...
**Q5 What's the difference between a histogram and a bar plot?** Histogram uses bins- bar plots use categorical while histograms use continuous
...
wi1sd <- mean(body_masses >= mean_mass - sd_mass & body_masses <= mean_mass + sd_mass)

wi2sd <- mean((body_masses >= mean_mass - 2*sd_mass & body_masses <= mean_mass + 2*sd_mass))
...
```{r}
swath <- read_csv(here::here("week1", "data", "PISCO_kelpforest_swath.1.3.csv"))
```
...
Creating a visual of Giant Kelp in depth zones at the Channel Islands

```{r}
ci_swath <- swath %>% 
  filter(campus == "UCSB" & classcode == "MACPYRAD")
...
```{r}
kelp_sum <- ci_swath %>%
  filter(zone != "SPECIALO", stringr::str_detect(site, pattern = "SCI")) %>% 
  group_by(year, zone) %>% 
  mutate(stipes = (count * size)/18, kelp = mean(stipes)) %>% 
  ungroup()

kelp_plot <- ggplot(kelp_sum, aes(year, kelp, color = zone)) +
  geom_point() +
  geom_line() +
  scale_color_paletteer_d("nationalparkcolors::ChannelIslands") +
  labs(x = "Year",
       y = "Number of Kelp Stipes per 10 m^2",
       title = "Average Number Kelp Stipes at Santa Cruz Island")

ggsave(here::here("week1", "kelp_plot.png"), height = 5, width = 8)
```
